#!/usr/bin/env python3
"""Run an audio-visual learning experiment across multiple video corruption settings.

This script orchestrates the existing pipeline in three stages for each video
corruption setting:
...
The default sweep matches the requested experiment: 10, 15, 20, 25, and 30
video frames affected, while audio stays fixed at 15 dB.
Each run writes to its own output directory, which makes it easy to compare
how the model learns as video quality changes.

Example
-------
    python run_av_fusion_video_experiment.py --phoneme-csv phonemes_s1_aligned/phoneme_predictions.csv

    python run_av_fusion_video_experiment.py --phase prepare --skip-existing
"""
...
def format_video_tag(video_obstruct_frames: int) -> str:
    return f"vf_{video_obstruct_frames}f"
...
def build_run_plan(base_noise_dir: Path, base_result_dir: Path, video_obstruct_frames_values: Sequence[int], modality: str) -> List[VideoRun]:
    ...
    for video_obstruct_frames in video_obstruct_frames_values:
        ...
        plan.append(VideoRun(
            video_obstruct_frames=video_obstruct_frames,
            modality=modality,
            tag=tag,
            noise_dir=noise_dir,
...
def make_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Sweep train_av_fusion.py across fixed video corruption lengths.",
    ...
    p.add_argument("--video-obstruct-frames-values", type=int, nargs="+", default=[10, 15, 20, 25, 30],
                   help="Video obstruction lengths in frames to sweep. The default matches the requested experiment.")
    p.add_argument("--audio-snr-db", type=float, default=15.0,
                   help="Fixed audio SNR in dB used for every run.")
    p.add_argument("--skip-existing", action="store_true",
                   help="Skip a stage when its expected output already exists.")
    p.add_argument("--summary-csv", default="video_sweep_results/video_sweep_summary.csv")
    p.add_argument("--summary-json", default="video_sweep_results/video_sweep_summary.json")
    ...
    p.add_argument("--noise-base-dir", default="video_sweep_results/noise")
    p.add_argument("--result-base-dir", default="video_sweep_results/train")

    p.add_argument("--fps", type=float, default=25.0)
    p.add_argument("--sample-rate", type=int, default=16000)
    p.add_argument("--video-obstruct-jitter", type=int, default=0)
    ...
    p.add_argument("--dry-run-noise", action="store_true",
                   help="Pass --dry-run to add_av_noise.py so only one clip is generated per setting.")
...
def build_noise_command(args: argparse.Namespace, run: VideoRun) -> List[str]:
    cmd = [
        sys.executable,
        "add_av_noise.py",
        ...
        "--video-obstruct-frames", str(run.video_obstruct_frames),
        "--video-obstruct-jitter", str(args.video_obstruct_jitter),
        "--audio-snr-db-range", str(args.audio_snr_db), str(args.audio_snr_db),
...
def build_precompute_command(args: argparse.Namespace, run: VideoRun) -> List[str]:
    cmd = [
        sys.executable,
...
def build_train_command(args: argparse.Namespace, run: VideoRun) -> List[str]:
    cmd = [
        sys.executable,
        "train_av_fusion.py",
...
def summarise_run(run: VideoRun) -> Dict[str, Any]:
    summary: Dict[str, Any] = {
        "video_obstruct_frames": run.video_obstruct_frames,
...
def main() -> None:
    ...
    if args.summary_csv == "video_sweep_results/video_sweep_summary.csv":
        args.summary_csv = f"video_sweep_results/{args.modality}_video_sweep_summary.csv"
    if args.summary_json == "video_sweep_results/video_sweep_summary.json":
        args.summary_json = f"video_sweep_results/{args.modality}_video_sweep_summary.json"

    runs = build_run_plan(base_noise_dir, base_result_dir, args.video_obstruct_frames_values, args.modality)

    summary_rows: List[Dict[str, Any]] = []

    for run in runs:
        log.info("=== Video obstruction %d frame(s) (%s, %s) ===", run.video_obstruct_frames, run.tag, run.modality)
    ...
    log.info("Finished %d video corruption run(s).", len(summary_rows))
