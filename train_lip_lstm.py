...
def load_aligned_phoneme_targets(csv_path: Path) -> Dict[str, AlignedPhonemeRecord]:
    # The aligned CSV is the training contract: labels are matched to videos
    # later by their normalized GRID stem.
    ...
    def __getitem__(self, index: int) -> Dict[str, Any]:
        ...
        if self.is_training:
            ...
            if random.random() < 0.4:
                ...
                video = v2.functional.adjust_brightness(video, brightness_factor)
                video = v2.functional.adjust_contrast(video, contrast_factor)
                video = v2.functional.adjust_saturation(video, saturation_factor)
...
class GraphAwareFrameCrossEntropyLoss(nn.Module):
    ...
    def __init__(self, vocab: Sequence[str], blank_id: int = 0, blank_penalty: float = 0.5, insertion_penalty: float = 1.0):
        ...
        self.insertion_penalty = float(insertion_penalty)
...
def main() -> None:
    ...
    logger.info(
        "Regularisation: blank_penalty=%.2f  insertion_penalty=%.2f  weight_decay=%.2e  "
        "projector_dropout=0.45  lstm_output_dropout=0.45",
        args.blank_penalty, args.insertion_penalty, args.weight_decay,
    )
